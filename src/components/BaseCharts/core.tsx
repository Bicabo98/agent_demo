...
export default class EChartsReactCore extends PureComponent<EChartsReactProps> {
  ...
  private bindEvents(
    instance: {
      on: (
        arg0: string,
        arg1: string | Record<string, any> | undefined,
        arg2: (param: any) => void,
      ) => void;
    },
    events: EChartsReactProps['onEvents'],
  ) {
    function _bindEvent(eventName: string, func: Function, query?: string | Record<string, any>) {
      // ignore the event config which not satisfy
      if (isString(eventName) && isFunction(func)) {
        // binding event
        instance.on(eventName, query, (param) => {
          func(param, instance);
        });
      }
    }
    ...
    for (const eventName in events) {
      if (Object.prototype.hasOwnProperty.call(events, eventName)) {
        _bindEvent(eventName, events[eventName].function, events[eventName].query);
      }
    }
  }
